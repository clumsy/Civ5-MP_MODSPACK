...
--Temple of Jerusalem
--------------------------------------
-- Temple of Jerusalem Vars
iJerusalemBuilding = GameInfoTypes["BUILDING_LEUGI_JERUSALEM_TEMPLE"]
iJerusalemMagisCost = 2
iJerusalemFaithCost = 800
---------------------------------------

local Decisions_Israel_Temple_Jerusalem = {}
	Decisions_Israel_Temple_Jerusalem.Name = "TXT_KEY_DECISIONS_LEUGI_ISRAEL_TEMPLE"
	Decisions_Israel_Temple_Jerusalem.Desc = "TXT_KEY_DECISIONS_LEUGI_ISRAEL_TEMPLE_DESC"
	Decisions_Israel_Temple_Jerusalem.Pedia = "TXT_KEY_BUILDING_LEUGI_JERUSALEM_TEMPLE"
	HookDecisionCivilizationIcon(Decisions_Israel_Temple_Jerusalem, "CIVILIZATION_LEUGI_ISRAEL")
	Decisions_Israel_Temple_Jerusalem.Weight = nil
	Decisions_Israel_Temple_Jerusalem.CanFunc = (
		function(pPlayer)	
			-- Check Civ
			if (pPlayer:GetCivilizationType() ~= GameInfoTypes["CIVILIZATION_LEUGI_ISRAEL"]) then return false, false end
			-- Check if the player has a temple of Jerusalem already
			if (pPlayer:CountNumBuildings(iJerusalemBuilding) > 0) then
				-- Build enacted description (only for humans)
				if pPlayer:IsHuman() then
					Decisions_Israel_Temple_Jerusalem.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_LEUGI_ISRAEL_TEMPLE_DESC_ENACTED_DESC")
				end
				return false, false, true
			end	
			JerusalemEnactable = true
			-- Requirements
			---- Get the Original Capital
			for pCity in pPlayer:Cities() do
				if pCity:IsCapital() then
					if not pCity:IsOriginalCapital() then
						JerusalemEnactable = false
					end
				end
			end
			...
			-- Costs
			---- Magistrates
			if (pPlayer:GetNumResourceAvailable(iMagistrate, false) < iJerusalemMagisCost) then JerusalemEnactable = false end	
			---- Faith
			local iCost = math.ceil((iJerusalemFaithCost) * iMod)
			if (pPlayer:GetFaith() < iCost) then JerusalemEnactable = false end
			-- Build the Description 
			Decisions_Israel_Temple_Jerusalem.Desc = Locale.ConvertTextKey("TXT_KEY_DECISIONS_LEUGI_ISRAEL_TEMPLE_DESC", iJerusalemMagisCost, iCost)
			if JerusalemEnactable == false then
				return true, false 
			end
			-- Allow the decision!
			return true, true
	...
	Decisions_Israel_Temple_Jerusalem.DoFunc = (
	function(pPlayer)
		-- Pay the price
		---- Magistrates
		pPlayer:ChangeNumResourceTotal(iMagistrate, -iJerusalemMagisCost)
		---- Faith
		local iCost = math.ceil((iJerusalemFaithCost) * iMod)
		...
		for pCity in pPlayer:Cities() do
				if pCity:IsCapital() then
					pCity:SetNumRealBuilding(iJerusalemBuilding, 1)
				end
			end
		-- Save the Decision
		save(pPlayer, "Decisions_Israel_Temple_Jerusalem", true)
	end
	)

Decisions_AddCivilisationSpecific(GameInfoTypes["CIVILIZATION_LEUGI_ISRAEL"], "Decisions_Israel_Temple_Jerusalem", Decisions_Israel_Temple_Jerusalem)
