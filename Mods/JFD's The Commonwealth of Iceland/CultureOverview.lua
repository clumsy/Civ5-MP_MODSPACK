...
function RefreshYourCulture()
    ...
    local playerCivilization = GameInfo.Civilizations[player:GetCivilizationType()];
...
function RefreshCultureVictory()
	...
	for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do	
		...
		if (player:IsAlive() and not player:IsMinorCiv() and activeTeam:IsHasMet(player:GetTeam())) then
			local row = {};
			
			row.strPlayerName = Locale.Lookup(player:GetCivilizationShortDescriptionKey()); 
    	
    		local iNumInfluential = player:GetNumCivsInfluentialOn();
    		local iNumToBeInfluential = player:GetNumCivsToBeInfluentialOn();
			...
			if (iIdeology ~= PolicyBranchTypes.NO_POLICY_BRANCH_TYPE) then
				...
				row.strPublicOpinionToolTip = player:GetPublicOpinionTooltip();
				...
				row.strPublicOpinionUnhappinessToolTip = player:GetPublicOpinionUnhappinessTooltip();
				
				local iExcessHappiness = player:GetExcessHappiness();
	...
	for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do	
		...
		if (player:IsAlive() and not player:IsMinorCiv() and activeTeam:IsHasMet(player:GetTeam())) then
			table.insert(g_InfluenceByPlayer.Players, {
				Text = Locale.Lookup(player:GetCivilizationShortDescriptionKey()),
				Id = player:GetID()
			});	
		end
	end
		...
		if (player:IsAlive() and not player:IsMinorCiv()) then
			...
			if (iPlayer ~= g_iSelectedPlayerID) then
    			...
    			if (iInfluenceLevel ~= InfluenceLevelTypes.NO_INFLUENCE_LEVEL) then
					...
					local iCulture = player:GetJONSCultureEverGenerated();
					local iPercent = 0;
					...
					if(activeTeam:IsHasMet(player:GetTeam())) then
						...
						playerInfluence.PlayerName = Locale.Lookup(player:GetCivilizationShortDescriptionKey());
						playerInfluence.InfluenceToolTip = Locale.Lookup("TXT_KEY_CO_THIRD_PARTY_CULTURE_INFLUENCE", pSelectedPlayer:GetCivilizationShortDescriptionKey(), iPercent * 100, player:GetCivilizationShortDescriptionKey());
					else
